...
function cloneState(state: GameState): GameState {
	return {
		...
		bomb: state.bomb ? { ...state.bomb, rect: { ...state.bomb.rect } } : null,
...
export function createGame(random: RandomSource = Math.random): GameState {
	return {
		...
		bomb: null,
		towers: Array.from({ length: TOWER_COUNT }, (_, index) =>
			createTower(index, random),
		),
...
export function stepGame(
	...
	if (
		next.towers.some(
			(tower) =>
				tower.respawnRemaining === 0 && tower.height >= TOWER_HEIGHT_CAP,
		)
	) {
	...
	if (
		next.towers.some(
			(tower) =>
				tower.respawnRemaining === 0 && overlaps(next.ship.rect, tower.rect),
		)
	) {
